...
//        throw new NotImplementedException();
        Action action;
        Stack<Status>statusList=new Stack<>();
        Stack<Symbol>symbolList=new Stack<>();
        statusList.push(this.lrTable.getInit());
        symbolList.push(new Symbol(Token.eof()));
        while (!tokenList.isEmpty()){
            action=lrTable.getAction(statusList.peek(),tokenList.get(0));
            if(action.getKind()==Action.ActionKind.Accept){
                callWhenInAccept(statusList.peek());
                tokenList.remove(0);
            }
            if(action.getKind()==Action.ActionKind.Shift){
                symbolList.push(new Symbol(tokenList.get(0)));
                statusList.push(action.getStatus());
                callWhenInShift(statusList.peek(),tokenList.get(0));
                tokenList.remove(0);
            }
            if(action.getKind()==Action.ActionKind.Reduce){
                for (int i = 0; i < action.getProduction().body().size(); i++) {
                    symbolList.pop();
                    statusList.pop();
                }
                symbolList.push(new Symbol(action.getProduction().head()));
                statusList.push(lrTable.getGoto(statusList.peek(),action.getProduction().head()));
                callWhenInReduce(statusList.peek(),action.getProduction());
            }
        }
    }
}
